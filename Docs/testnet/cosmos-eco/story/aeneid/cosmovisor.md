...
```
cd $HOME
mkdir -p $HOME/.story/story/cosmovisor/upgrades/v1.4.2/bin
cp $HOME/go/bin/story $HOME/.story/story/cosmovisor/upgrades/v1.4.2/bin
```

#### Check version:

```
$HOME/.story/story/cosmovisor/upgrades/v1.4.2/bin/story version
```
...
Git Commit    19af5ad
Git Timestamp 2025-10-30T21:37:46Z
```

#### Check upgrade info

```
cat $HOME/.story/story/cosmovisor/upgrades/v1.4.2/upgrade-info.json
```

result: `{`"name":"v1.4.2","time":"0001-01-01T00:00:00Z","height":10886688`}`

#### Create upgrade simlink:

```
echo '{"name":"v1.4.2","time":"0001-01-01T00:00:00Z","height":10886688}' > $HOME/.story/story/cosmovisor/upgrades/v1.4.2/upgrade-info.json
```
...
result: `lrwxrwxrwx 1 story story 15 May 5 17:20 /home/story/.story/story/cosmovisor/current -> upgrades/v1.4.0`
...
```
cosmovisor add-upgrade v1.4.2 $HOME/.story/story/cosmovisor/upgrades/v1.4.2/bin/story --force --upgrade-height 10886688
```

result:

```
10:13AM INF Using /home/story/.story/story/cosmovisor/upgrades/v1.4.2/bin/story for v1.4.2 upgrade module=cosmovisor
10:13AM INF Upgrade binary located at /home/story/.story/story/cosmovisor/upgrades/v1.4.2/bin/story module=cosmovisor
10:13AM INF /home/story/.story/story/data/upgrade-info.json created, v1.4.2 upgrade binary will switch at height 10886688 module=cosmovisor
```

### Create service file:

```
sudo tee /etc/systemd/system/storytest.service > /dev/null << EOF
[Unit]
...
```
story init --moniker $MONIKER --network aeneid
```

### set seeds and peers

```
SEEDS="944e8889ecd7c13623ef1081aae4555d6f525041@b1-b.aeneid-devnet.storyrpc.io:26656"
...
sudo systemctl enable storytest story-geth
...
**From now please do not stop and restart node before block** 10.886.688 **. Because it will run forcely with new binary.**
...
```
rm -rf $HOME/.story/story/cosmovisor/upgrades/v1.4.2
```
...
```
ln -s $HOME/.story/story/cosmovisor/upgrades/v1.4.0 $HOME/.story/story/cosmovisor/current
```

#### _Finally Start node & setup v1.4.2 with Cosmosvisor again._
